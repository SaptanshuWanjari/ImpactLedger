...
function createSampleInvoicePdf(input) {
...
function main() {
  const outputArg = process.argv[2] || "./tmp/sample-donation-receipt.pdf";
  const outputPath = path.resolve(process.cwd(), outputArg);

  const samplePdf = createSampleInvoicePdf({
    donationId: "DON-SAMPLE-2026-0001",
    donorName: "Aarav Mehta",
    donorEmail: "aarav.mehta@example.com",
    campaignTitle: "Cyclone Emergency Relief",
    amountInInr: 5000,
    donatedAtIso: new Date().toISOString(),
    orgName: "Impact Ledger",
    receiptUrl: "https://impact-ledger.example/donate/success?donationId=DON-SAMPLE-2026-0001",
  });

  fs.mkdirSync(path.dirname(outputPath), { recursive: true });
  fs.writeFileSync(outputPath, samplePdf);

  console.log(`Sample invoice PDF generated at: ${outputPath}`);
}
