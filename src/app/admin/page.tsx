...
export default function AdminDashboard() {
  ...
  async function runManualAction(donationId: string, action: "verify" | "send_receipt") {
          ...
          {manualLoading ? (
            ...
            <div className="space-y-3">
              {manualDonations.map((item) => {
                ...
                return (
                  <div key={item.id} className="rounded-xl border border-muted px-4 py-3 bg-white">
                      ...
                      <div className="flex flex-wrap items-center gap-2">
                        <span
                          className={cn(
                            ...
                            isSucceeded ? "bg-green-50 text-green-700" : "bg-amber-50 text-amber-700",
                          )}
                        >
                          {item.status}
                        </span>
                        ...
                        >
                          {isBusy ? "Working..." : "Verify + Send Receipt"}
                        </button>
                        <button
                          type="button"
                          disabled={isBusy || !isSucceeded}
                          onClick={() => runManualAction(item.id, "send_receipt")}
                          className="rounded-lg border border-muted px-3 py-1.5 text-xs font-semibold disabled:opacity-60"
                        >
                          Send Receipt
                        </button>
                      </div>
